...
impl Config {
    pub fn new(args:&[String])->Config {
        if args.len() < 3 {
            panic!("not enough arguments, there should be two");
        }
        let query = args[1].clone();
        let file_path = args[2].clone();
        let ignore_case = env::var("IGNORE_CASE").is_ok(); // var  will return a Result, 
                                                                     // if no error is thrown 
                                                                     // an Ok(val) will return true, the contents of the Ok are of no importance
